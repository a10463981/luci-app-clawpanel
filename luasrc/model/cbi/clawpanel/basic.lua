...
local check_system_url = luci.dispatcher.build_url("admin", "services", "clawpanel", "check_system")
	...
	html[#html+1] = '<div style="font-size:12px;color:#666;margin-bottom:4px;">📝 自定义路径（手动输入）:</div>'
	...
	html[#html+1] = '<button class="btn cbi-button cbi-button-apply" type="button" onclick="cpConfirmSetup()" style="min-width:80px;" id="cp-confirm-btn">开始安装</button>'
	html[#html+1] = '</div>'
	html[#html+1] = '</div></div>'
	...
	-- 加载可用挂载点
	html[#html+1] = 'var _cpSelectedMount="";'
	html[#html+1] = 'function cpLoadMounts(){'
	html[#html+1] = 'var el=document.getElementById("cp-mount-list");'
	html[#html+1] = 'var customWrap=document.getElementById("cp-custom-path-wrap");'
	html[#html+1] = 'if(!el)return;'
	html[#html+1] = 'el.innerHTML="<div style=\\"color:#888;font-size:12px;\\">⏳ 正在检测存储设备...</div>";'
	html[#html+1] = '(new XHR()).get("' .. mounts_url .. '",null,function(x){'
	html[#html+1] = 'try{'
	html[#html+1] = 'var r=JSON.parse(x.responseText);'
	html[#html+1] = 'if(!r.mounts||r.mounts.length===0){'
	html[#html+1] = 'el.innerHTML="<div style=\\"color:#e36209;font-size:12px;padding:6px 0;\\">⚠️ 未检测到外置存储挂载点<br/><span style=\\"color:#888;font-size:11px;\\">请先在「系统 → 挂载点」中挂载 USB 硬盘或外部存储，再安装 ClawPanel。</span></div>";'
	html[#html+1] = 'customWrap.style.display="block";'
	html[#html+1] = '}else{'
	html[#html+1] = 'var h="<div style=\\"font-size:12px;color:#888;margin-bottom:6px;\\">⬇️ 选择安装位置（推荐第一个挂载点）：</div>";'
	html[#html+1] = 'r.mounts.forEach(function(m,i){'
	html[#html+1] = 'var checked=i===0?"checked":"";'
	html[#html+1] = 'var rec=i===0?" <span style=\\"background:#1a7f37;color:#fff;padding:1px 6px;border-radius:3px;font-size:10px;\\">推荐</span>":"";'
	html[#html+1] = 'h+=\'<label style="display:flex;align-items:flex-start;gap:8px;padding:10px 12px;border:2px solid \'+(i===0?"#4a90d9":"#e0e0e0")+\';border-radius:8px;cursor:pointer;background:\'+(i===0?"#f0f7ff":"#fff")+\';">\';'
	html[#html+1] = 'h+=\'<input type="radio" name="cp-mount" value="\'+m.mount+\'" style="margin-top:2px;" \'+(i===0?"checked":"")+\' onclick="cpSelectMount(\\\'\\'+m.mount+\\\'\\')">\';'
	html[#html+1] = 'h+=\'<div><strong>\'+m.mount+\'</strong>\'+rec+\'<div style="font-size:11px;color:#666;margin-top:2px;">可用空间: <span style="color:\'+(m.avail_mb>=500?"#1a7f37":"#e36209")+\';font-weight:600;">\'+m.size+\'</span> | 文件系统: \'+(m.fs||"unknown")+\'</div></div>\';'
	html[#html+1] = 'h+="</label>";'
	html[#html+1] = '});'
	html[#html+1] = 'el.innerHTML=h;'
	html[#html+1] = 'if(r.mounts.length>0){_cpSelectedMount=r.mounts[0].mount;}'
	html[#html+1] = 'customWrap.style.display="block";'
	html[#html+1] = 'document.getElementById("cp-install-path").value=_cpSelectedMount;'
	html[#html+1] = '}'
	html[#html+1] = '}catch(e){'
	html[#html+1] = 'el.innerHTML="<div style=\\"color:#cf222e;font-size:12px;\\">⚠️ 检测失败: "+e.message+"</div>";'
	html[#html+1] = 'customWrap.style.display="block";'
	html[#html+1] = '}'
	html[#html+1] = '});'
	html[#html+1] = '}'
	...
	-- 选中挂载点
	html[#html+1] = 'function cpSelectMount(path){'
	html[#html+1] = '_cpSelectedMount=path;'
	html[#html+1] = 'document.getElementById("cp-install-path").value=path;'
	html[#html+1] = '}'
	...
	html[#html+1] = 'if(!path){alert("请选择或输入安装路径");return;}'

	-- 危险路径校验
	html[#html+1] = 'var dangerous=["/overlay","/rom","/boot","/proc","/sys","/dev","/run","/tmp","/var","/etc","/root","/usr","/bin","/sbin","/lib","/opt","/"];'
	html[#html+1] = 'var isSys=false;'
	html[#html+1] = 'dangerous.forEach(function(d){if(path===d||path.startsWith(d+"/"))isSys=true;});'
	html[#html+1] = 'if(isSys){'
	html[#html+1] = 'alert("错误：禁止将 ClawPanel 安装到系统分区！\\n\\n请选择外置存储挂载点（如 /mnt/sda1）。\\n系统分区空间有限且重装后会丢失数据！");'
	html[#html+1] = 'return;'
	html[#html+1] = '}'
	...
	html[#html+1] = 'var r=JSON.parse(x.responseText);'
	html[#html+1] = 'logEl.textContent+="════════════════════════════════════════\\n";'
	html[#html+1] = 'logEl.textContent+="🔍 安装路径检查\\n";'
	html[#html+1] = 'logEl.textContent+="════════════════════════════════════════\\n";'
	html[#html+1] = 'logEl.textContent+="安装路径: "+path+"\\n";'
	...
	html[#html+1] = 'return;'
	html[#html+1] = '}'
	html[#html+1] = 'logEl.textContent+="✅ 检查通过，开始安装...\\n\\n";'
	html[#html+1] = 'var verParam=version?"&version="+encodeURIComponent(version):"";'
	html[#html+1] = '(new XHR()).get("' .. ctl_url .. '?action=setup&install_path="+encodeURIComponent(path)+verParam,null,function(x){'
